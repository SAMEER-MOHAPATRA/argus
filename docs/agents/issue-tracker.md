...
**PRs as a request surface: no.** This is a solo repo right now — `/triage` treats PRs as ordinary
in-flight work, not an incoming-request surface. Flip to `yes` if the repo ever gets outside
contributors.
